...
A local stand-in for the backend, for testing gateway V2 end to end. It is for dev/test only and is not a production server. None of the pages below has TLS, and the server-sim page has no login.

| Service | What it does | Open |
|---|---|---|
| `server-sim` | Speaks the V2 wire contract ([Server_Design_Spec.md](../Server_Design_Spec.md)), stores readings in InfluxDB and **acks only after the write succeeds**. Its page shows status and holds the config (see below). | http://localhost:9000 (no login) |
...
- **Status:** one row per gateway, showing:
  ...
  Recent gaps are listed below the table. These counters live in memory and reset when server-sim restarts.
...
- **MaxGate400:** use `tcp://<this PC's LAN or Tailscale address>:1883`. This needs three things:
  - the device running V2, since V1 sends the old `entries` format, which server-sim rejects;
  - Windows Firewall allowing inbound TCP 1883;
  - a decision about exposure: anonymous EMQX, and the server-sim page with no login, become reachable by anything on that network.
...
- **Grafana variables:** the dashboard has a *Bucket* selector (for the case where you change the bucket on the server-sim page) and a *Gateway* selector.

## Limits

- server-sim remembers which chunks it has seen and each stream's last `seq` only in memory. After it restarts, a resent chunk is written to InfluxDB again. That is harmless, because the write is idempotent. The first chunk it sees after a restart cannot be checked for a gap.
